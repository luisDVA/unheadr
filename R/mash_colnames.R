...
#' @param sliding_headers If TRUE, empty values in the header row be filled
...
mash_colnames <- function(df,
  ...
  if (n_name_rows==0){stop("Setting n_name_rows to 0 implies that names are not broken")}
  ...
  long_namesdf <- dplyr::mutate(long_namesdf, col_pos = rep(
    seq_len(dplyr::n_distinct(ogcols)),
    dplyr::n() / dplyr::n_distinct(ogcols)
  ))

  if (sliding_headers) {
    longnamesdf_grouped <- dplyr::group_by(long_namesdf, ROW_ID)
    longnamesdf_grouped_filled <- tidyr::fill(longnamesdf_grouped, values, .direction = "down")
    long_namesdf <- dplyr::ungroup(longnamesdf_grouped_filled)
  }

  # Collapse the column names together into single elements
  summrzddf <-
    dplyr::summarize(dplyr::group_by(long_namesdf, col_pos),
                     values = paste0(values, collapse = sep),
                     .groups = "keep"
    )
